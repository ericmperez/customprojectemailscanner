...
export async function PATCH(
  ...
  try {
    ...
    const notes = formatNoteWithAttribution(rawNotes || 'Aprobada', userName);

    const licitacion = await licitacionesService.updateApprovalStatus(orgId, id, 'approved', notes);

    logActivity(orgId, 'approve', id, licitacion?.title ?? null, userName);

    return NextResponse.json({ success: true, data: licitacion });
  } catch (error) {
    console.error(`Error approving licitacion ${id}:`, error);
    return NextResponse.json(
