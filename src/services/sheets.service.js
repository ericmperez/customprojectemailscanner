    ...
    try {
      ...
      if (!values || values.length === 0) {
        ...
        if (currentHeaders.length < HEADERS.length) {
          await this._writeHeaders();
          logger.info('Updated sheet headers with new columns');
        } else {
          ...
          if (needsUpdate) {
            await this._writeHeaders();
            logger.info('Normalized sheet headers');
          }
        }
      }
      ...
      logger.error('Error ensuring sheet initialization:', error);
      throw error;
    }
  }
  ...
  async getLicitaciones(filters = {}) {
    ...
    if (updatePromises.length > 0) {
      Promise.all(updatePromises).catch(err => 
        logger.error('Some auto-reject updates failed:', err)
      );
    }
