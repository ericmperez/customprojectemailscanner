...
import { AISettingsTab } from '@/components/modals/AISettingsTab';
...
const DEFAULT_SETTINGS: ConfidenceFieldSettings = {
  ...
  optional: [
    'title', 'summary', 'category', 'siteVisitDate', 'siteVisitTime',
    'visitLocation', 'contactName', 'biddingCloseTime', 'estimatedValue',
  ],
  ignored: [],
};

type FieldCategory = 'critical' | 'optional' | 'ignored';
type SectionId = 'confidence' | 'instructions';
...
export function AISettingsPanel() {
  ...
  const [section, setSection] = useState<SectionId>('confidence');
  ...
  const hasChanges =
    settings &&
    JSON.stringify({
      critical: [...draft.critical].sort(),
      optional: [...draft.optional].sort(),
      ignored: [...draft.ignored].sort(),
    }) !==
      JSON.stringify({
        critical: [...settings.critical].sort(),
        optional: [...settings.optional].sort(),
        ignored: [...settings.ignored].sort(),
      });

  const SECTIONS: { id: SectionId; label: string }[] = [
    { id: 'confidence', label: 'Campos de confianza' },
    { id: 'instructions', label: 'Instrucciones AI' },
  ];
  ...
  return (
    <div className="space-y-4">
                    ...
                    <tr key={field} className="border-b last:border-0">
                      ...
                      <td className="py-2">
                        <select
                          ...
                          value={category}
                          onChange={(e) => handleCategoryChange(field, e.target.value as FieldCategory)}
                          disabled={saving}
                        >
      ...
      {section === 'instructions' && <AISettingsTab />}
    </div>
  );
}
